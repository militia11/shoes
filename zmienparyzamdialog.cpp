...
#include <QKeyEvent>
#include <QMessageBox>
zmienParyZamDialog::zmienParyZamDialog(klienciDialog *dialog, BazaDanychManager *bm, QWidget *parent) :
    QDialog(parent),
    dbmanager(bm),
    dialog(dialog),stModel(nullptr),
...
void zmienParyZamDialog::on_pushButtonModel_2_clicked() {
...
void zmienParyZamDialog::showEvent(QShowEvent *e) {
    wyczyscPola();
    ...
    idModelu = dbmanager->getZamModeleID(curId);
    delete abmodel;
    if (stModel) {
...
void zmienParyZamDialog::hideEvent(QHideEvent *e) {
    curId = -1;
}

void zmienParyZamDialog::acceptRoz() {
    ...
    if(ui->labelKlient->text() == QString("")) {
        QMessageBox::information(this, "NIE WYBRANO KLIENTA", " <FONT COLOR='#0f00f0'>Proszę wskazać nowego klienta dla zamówienia.", QMessageBox::Ok);
    } else {
        if(wysKomunikat) {
            rozmiaryx.clear();
...
QList<QStandardItem *> zmienParyZamDialog::zwrocWierszModel() {
    QList<QStandardItem *> rzad;
    ...
    rzad.append(new QStandardItem(stModel->index(0,48).data(Qt::DisplayRole).toString()));
//    for (int i =0; i< stModel->columnCount(); i++) {
//        qDebug()<<i << stModel->index(0,i).data(Qt::DisplayRole).toString();
//    }
    return rzad;
}

void zmienParyZamDialog::wyczyscPola() {
    rozmiaryx.clear();
    ui->lineEdit36->clear();
    ui->lineEdit37->clear();
    ui->lineEdit38->clear();
...
void zmienParyZamDialog::obliczSume() {
...
void zmienParyZamDialog::obliczSumeGora() {
    int suma = 0;
    if(stModel) {
        for (int i = 8; i < 23; i++) {
...
bool zmienParyZamDialog::eventFilter(QObject *object, QEvent *event) {
...
int zmienParyZamDialog::getIdModelu() const {
    return idModelu;
}

int zmienParyZamDialog::getCurId() const {
    return curId;
}

void zmienParyZamDialog::setCurId(int value) {
    curId = value;
}
